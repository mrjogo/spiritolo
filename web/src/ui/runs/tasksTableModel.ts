...
//   - done + hold      → constructive bulk "Apply" (write the held results)
//   - anything running → inspect-only (you can look, not mutate mid-flight)
export type BatchMode = 'remove' | 'apply' | 'inspect';

export function batchMode(runState: RunState, applyMode: ApplyMode): BatchMode {
  if (runState === 'draft') return 'remove';
  if (runState === 'done' && applyMode === 'hold') return 'apply';
  return 'inspect';
}
