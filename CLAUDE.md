...
- **Worker — the always-on daemon over the `jobs` queue.** Claims a job (`FOR UPDATE SKIP LOCKED`), dispatches it to its stage_fn with the run's own `ProviderChain` (a single LLM tier + a per-job cost cap), heartbeats while it runs, then finalizes. **The run picks the model:** its `llm_provider` / `llm_model` (chosen at run assembly via `set_run_llm`, the same fields the cost gate reads) select exactly one LLM provider, built per job from the env's API keys — a run that names no provider (or whose key is absent) runs deterministic-only. The deterministic alias/lexical tiers live in the stage code itself, and `pack_size` (items per LLM call) is a per-stage code constant (`STAGE_PACK_SIZE` in `worker/providers.py`). There is no external provider-chain config file.
