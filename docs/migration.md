...
```bash
export SUPABASE_DB_URL="postgresql://postgres.atvlzbgrquiseczzeczn:<pw>@aws-0-<region>.pooler.supabase.com:5432/postgres"
...
eval "$(railway bucket credentials)"  # loads the bucket's AWS_* creds
export S3_ENDPOINT="$AWS_ENDPOINT_URL" S3_REGION=auto \
