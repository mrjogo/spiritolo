...
Classify all unclassified URLs in `data/scraper.db` by URL slug into one of five categories.
...
- Each subagent should:
  ...
  5. Report back only the counts: `{site}: {n} likely_drink_recipe, {n} likely_food_recipe, {n} likely_drink_article, {n} likely_food_article, {n} likely_junk`
