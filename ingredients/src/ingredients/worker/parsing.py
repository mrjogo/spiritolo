...
landed, ``worker`` became a package; this module keeps the original per-recipe
parsing helper, re-exported from ``ingredients.worker`` so existing importers
(``ingredients.cli``, ``tests/test_worker.py``) are unaffected.
"""
